...
/*
 * This function performs a sox analysis and is a 
 * drop in replacement for the libsox main() function
 * that is executed in the sox binary 
...
int performSox(const char* audio_path, 
                const char* out_path) {
  ...
  assert(in = sox_open_read(audio_path, NULL, NULL, NULL));

  /* Open the output file; we must specify the output signal characteristics.
   * Since we are using only simple effects, they are the same as the input
   * file characteristics */
  assert(out = sox_open_write(out_path, &in->signal, NULL, NULL, NULL, NULL));
